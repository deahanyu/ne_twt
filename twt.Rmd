...
for (i in colnames(dt)) {
  ...
  } else if(length(grep("[.]",i))>0 || length(grep("Mortality",i))>0 || length(grep("Sentiment",i))>0 || length(grep("total_twts",i))>0||length(grep("recrecen",i))>0||length(grep("Grocery_Store_AvgDistance",i))>0 || length(grep("Voter_Turnout",i))>0||length(grep("Parkland",i))>0||length(grep("Disability",i))>0) {
...
creatingNewColumnNames<-function(xxx,yname=1){
  ...
  if (yname==1){
    ...
    for (i in colnames(xxx)){
    if (i=="LOG10_PCT_18_to_29_09_13"){
      newNames<-c(newNames,"PCT_Ages")
    }
    else {
      split.pos <- gregexpr("_",i)[[1]]
      split.length <- attr(split.pos, "match.length")
...
ydt<-dt[,ys,with=FALSE]
colnames(ydt)<-creatingNewColumnNames(ydt)
k<-c()
for (i in colnames(ydt)){
...
for (j in c("score","healthy","unhealthy","alcohol")){
   thiss<-which(k%in%j)
   pairs(ydt[,thiss,with=FALSE],lower.panel=panel_cor)
}
```
...
for (i in 1:length(colnames(ydt))){
  if(colnames(ydt)[i]=="tract B alcohol"||colnames(ydt)[i]=="user B alcohol"){
    pairs(cbind(ydt[,i,with=FALSE],xtable_alc1),lower.panel=panel_cor)
  }else{
    pairs(cbind(ydt[,i,with=FALSE],xtable_food1),lower.panel=panel_cor)
  }
}
```
